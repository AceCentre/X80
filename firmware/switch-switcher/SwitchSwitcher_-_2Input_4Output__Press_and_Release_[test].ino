...
boolean PressModeOn = false; // this will determine if the program will work in PRESS mode
boolean ReleaseModeOn = true; // this will determine if the program will work in RELEASE mode
...
// ################## OUTPUT 1 #################
  if (button1Val == LOW && button1Last == HIGH && btn1DnTime > long(debounce) && output1 == true){
    event1();
    btn1UpTime = millis();
  }

  if (button2Val == LOW && button2Last == HIGH && btn2DnTime > long(debounce) && output1 == true){
    event5();
    btn2UpTime = millis();
  }
...
// ################## OUTPUT 2 #################
  if (button1Val == LOW && button1Last == HIGH && btn1DnTime > long(debounce) && output2 == true){
    event2();
    btn1UpTime = millis();
  }

  if (button2Val == LOW && button2Last == HIGH && btn2DnTime > long(debounce) && output2 == true){
    event6();
    btn2UpTime = millis();
  }
  
// ################## OUTPUT 3 #################
  if (button1Val == LOW && button1Last == HIGH && btn1DnTime > long(debounce) && output3 == true){
    event3();
    btn1UpTime = millis();
  }

  if (button2Val == LOW && button2Last == HIGH && btn2DnTime > long(debounce) && output3 == true){
    event7();
    btn2UpTime = millis();
  }

// ################## OUTPUT 4 #################
  if (button1Val == LOW && button1Last == HIGH && btn1DnTime > long(debounce) && output4 == true){
    event4();
    btn1UpTime = millis();
  }

  if (button2Val == LOW && button2Last == HIGH && btn2DnTime > long(debounce) && output4 == true){
    event8();
    btn2UpTime = millis();
  }
...
// Test for buttons release and store the up time
    ...
    if (button1Val == HIGH && button1Last == LOW && (millis() - btn1DnTime) > long(debounce) && output1 == true)
      {
      if (ignore1Up == false) event1();
      else ignore1Up = false;
      btn1UpTime = millis();
      }


    if (button2Val == HIGH && button2Last == LOW && (millis() - btn2DnTime) > long(debounce) && output1 == true)
      {
      if (ignore2Up == false) event5();
...
// Test for buttons release and store the up time
    ...
    if (button1Val == HIGH && button1Last == LOW && (millis() - btn1DnTime) > long(debounce) && output2 == true)
     {
     if (ignore1Up == false) event2();
     else ignore1Up = false;
     btn1UpTime = millis();
     }

    if (button2Val == HIGH && button2Last == LOW && (millis() - btn2DnTime) > long(debounce) && output2 == true)
     {
     if (ignore2Up == false) event6();
     else ignore2Up = false;
     btn2UpTime = millis();
     }
...
// Test for buttons release and store the up time
    ...
    if (button1Val == HIGH && button1Last == LOW && (millis() - btn1DnTime) > long(debounce) && output3 == true)
     {
     if (ignore1Up == false) event3();
     else ignore1Up = false;
     btn1UpTime = millis();
     }

    if (button2Val == HIGH && button2Last == LOW && (millis() - btn2DnTime) > long(debounce) && output3 == true)
     {
     if (ignore2Up == false) event7();
     else ignore2Up = false;
     btn2UpTime = millis();
     }
...
// Test for buttons release and store the up time
    ...
    if (button1Val == HIGH && button1Last == LOW && (millis() - btn1DnTime) > long(debounce) && output4 == true)
      {
      if (ignore1Up == false) event4();
      else ignore1Up = false;
      btn1UpTime = millis();
      }

    if (button2Val == HIGH && button2Last == LOW && (millis() - btn2DnTime) > long(debounce) && output4 == true)
      {
      if (ignore2Up == false) event8();
      else ignore2Up = false;
      btn2UpTime = millis();
      }
